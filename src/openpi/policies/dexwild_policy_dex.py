...
def make_libero_example() -> dict:
    """Creates a random input example for the DexWild policy."""
    return {
        "observation/state": np.random.rand(8),
        "observation/right_pinky_image": np.random.randint(256, size=(320, 240, 3), dtype=np.uint8),
        "observation/right_thumb_image": np.random.randint(256, size=(320, 240, 3), dtype=np.uint8),
        "prompt": "do something",
    }
...
class DexwildInputs(transforms.DataTransformFn):
    """
    ...
    # Do not change this for your own dataset.
    model_type: _model.ModelType = _model.ModelType.PI0Dex

    def __call__(self, data: dict) -> dict:
        # We only mask padding for pi0 model, not pi0-FAST. Do not change this for your own dataset.
        mask_padding = self.model_type == _model.ModelType.PI0Dex
        ...
        inputs = {
            ...
            "image": {
                ...
                "left_wrist_0_rgb": right_thumb_image,
            },
            "image_mask": {
...
@dataclasses.dataclass(frozen=True)
class DexwildOutputs(transforms.DataTransformFn):
    """
