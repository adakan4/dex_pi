...
import openpi.models.pi0_dex as pi0_dex
import openpi.models.pi0_fast as pi0_fast
import openpi.models.tokenizer as _tokenizer
import openpi.policies.aloha_policy as aloha_policy
import openpi.policies.droid_policy as droid_policy
import openpi.policies.libero_policy as libero_policy
import openpi.policies.dexwild_policy as dexwild_policy
...
@dataclasses.dataclass(frozen=True)
class LeRobotDexwildDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "observation/right_pinky_image": "right_pinky_image",
                        "observation/right_thumb_image": "right_thumb_image",
                        "observation/state": "state",
                        "actions": "actions",
                        "prompt": "prompt",
                    }
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[dexwild_policy.DexwildInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[dexwild_policy.DexwildOutputs()],
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi0_dexwild_dex_lora",
        # Here is an example of loading a pi0 model for LoRA fine-tuning.
        model=pi0_dex.Pi0DexConfig(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotDexwildDataConfig(
            repo_id="adakan4/dexwild_toy_cleanup_gates",
            base_config=DataConfig(
                local_files_only=False,  # Set to True for local-only datasets.
        ...
        # for the given model config for LoRA finetuning. Just make sure it matches the model config
        # you chose above.
        freeze_filter=pi0_dex.Pi0DexConfig(
            paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"
        ).get_freeze_filter(),
        # Turn off EMA for LoRA finetuning.
        ema_decay=None,
    ...
    TrainConfig(
    name="pi0_fast_dexwild_low_mem_finetune",
    # Here is an example of loading a pi0-FAST model for LoRA finetuning.
    # For setting action_dim, action_horizon, and max_token_len, see the comments above.
    model=pi0_fast.Pi0FASTConfig(
